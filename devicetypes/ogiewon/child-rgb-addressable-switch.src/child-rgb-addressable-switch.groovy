...
// for the UI
metadata {
	definition (name: "Child RGB Addressable Switch", namespace: "ogiewon", author: "Allan (vseven) - based on code by Dan Ogorchock", ocfDeviceType: "oic.d.light", mnmn: "SmartThings", vid: "generic-rgbw-color-bulb") {
	capability "Switch"		
...
def adjustColor(colorInHEX) {
    ...
    sendData("color ${adjustedColor}")
    sendData("brightness ${level}")
}
...
def doPattern(patternName) {
    sendData("pattern ${patternName}")
}
...
def packers() { doPattern("packers") }
def rainbow() { doPattern("rainbow") }
